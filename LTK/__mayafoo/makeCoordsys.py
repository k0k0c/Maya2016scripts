...
cmds.setAttr ('eyesMouthRender_geoShape.rman__torattr___preShapeScript',"sendMatrix(\"coordsys\");sendMatrix(\"coordsys2\");",type="string")
...
cmds.namespace( add = selectionList + '_nmspc' )    
cmds.namespace( set = selectionList + '_nmspc' )
...
cmds.namespace( set = ':' )
cmds.namespace( mnr = True, rm = '%s_nmspc' % ( selectionList ) )

#make coordsys2 for eyesMouthRender_geo 

cmds.namespace( add = '%s_nmspc' % ( selectionList ) )    
cmds.namespace( set = '%s_nmspc' % ( selectionList ) )
